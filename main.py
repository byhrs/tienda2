...
#base de datos
##app.config['SQLALCHEMY_DATABASE_URI']='mysql+pymysql://root:root@localhost/gamestore3' #cadena de conexion

app.config['SQLALCHEMY_DATABASE_URI']='mysql+pymysql://root:aDDG-dHF56B1F6AaeheB2AG2dhdF3gh4@monorail.proxy.rlwy.net:14216/railway' #cadena de conexion
...
def juegos():
    ...
    return render_template('productos/juegos.html', games=games)
...
def registrarJuego():
    if current_user.is_admin() or current_user.is_vendedor(): 
        ...
        return render_template('productos/registrarJuego.html', cates=cates)
    return redirect(url_for('juegos'))
...
def actualizarJuego(numJuego):
    if current_user.is_admin() or current_user.is_vendedor():
        ...
        return render_template('/productos/actualizarJuego.html', game=j, cates=cates)
    return redirect(url_for('juegos'))
